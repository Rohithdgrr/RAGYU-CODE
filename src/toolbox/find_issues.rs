...
pub fn run(base: &Path, args: Args) -> anyhow::Result<String> {
    ...
    anyhow::ensure!(scope.is_dir(), "scope is not a directory: {}", scope.display());
    ...
    Ok(format!(
        ...
        args.issue, scope.display(), result.len(),
...
fn list_source_files(scope: &Path) -> Vec<std::path::PathBuf> {
    ...
    fn walk(dir: &Path, skip: &[&str], out: &mut Vec<std::path::PathBuf>) {
        ...
        for e in entries.flatten() {
            if skip.contains(&e.file_name().to_string_lossy().as_ref()) { continue; }
            let path = e.path();
            if path.is_dir() { walk(&path, skip, out); }
            else if let Some(ext) = path.extension().and_then(|x| x.to_str()) {
                if matches!(ext, "rs" | "py" | "js" | "ts" | "go") { out.push(path); }
            }
        }
    }
...
fn find_duplicates(scope: &Path, min_lines: usize) -> Vec<serde_json::Value> {
    ...
    for f in list_source_files(scope) {
        ...
        for (i, win) in content.lines().collect::<Vec<_>>().windows(min_lines).enumerate() {
            let key: String = win.iter().map(|l| l.trim()).collect::<Vec<_>>().join("|");
            if key.is_empty() { continue; }
            chunks.entry(key).or_default().push((f.display().to_string(), i + 1));
        }
    }
    ...
    for (block, locs) in chunks {
        ...
        if unique_files.len() < 2 { continue; }
...
fn find_deadcode(scope: &Path) -> Vec<serde_json::Value> {
    ...
    let mut out = Vec::new();
    for f in &files {
        if !f.extension().map_or(false, |e| e == "rs") { continue; }
        let Ok(content) = std::fs::read_to_string(f) else { continue };
            ...
            if let Some(idx) = line.find("pub fn ") {
                ...
                if files.len() > 1 {
                    let referenced_elsewhere = files.iter().any(|other| {
                        other != f && std::fs::read_to_string(other)
    ...
    for f in list_source_files(scope) {
        ...
        for (lineno, line) in content.lines().enumerate() {
            if let Some(rest) = line.trim_start().strip_prefix("use ") {
                let last = rest.rsplit("::").next().unwrap_or("").trim_end_matches(';').trim();
                let ident: String = last
                    ...
                    .collect();
                if ident.is_empty() || ident == "self" { continue; }
        ...
        for (i, line) in content.lines().enumerate() {
            if !in_fn {
                if let Some(idx) = line.find("fn ") {
                    ...
                    if first.map_or(true, |c| !c.is_alphanumeric() && c != '_') { continue; }
                ...
                if depth <= 0 && line.contains('}') {
                    let body: String = content.lines().take(i + 1).skip(fn_start).collect::<Vec<_>>().join("\n");
...
mod tests {
    ...
    fn finds_duplicate_blocks() {
        ...
        let args = Args { issue: Issue::Duplicates, path: None, min_lines: Some(5) };
