...
pub enum GitAction {
    Stash { sub: Option<String>, message: Option<String> },
    CreateBranch { name: String, from: Option<String> },
    Checkout { name: String },
    Rebase { upstream: String, interactive: Option<bool> },
    Merge { branch: String, no_ff: Option<bool> },
    RemoteAdd { name: String, url: String },
    Tag { name: String, sha: Option<String>, message: Option<String> },
}
...
pub async fn run(base: &Path, args: Args) -> anyhow::Result<String> {
    let argv: Vec<String> = match &args.action {
        GitAction::Stash { sub, message } => {
            ...
            if let Some(s) = sub { a.push(s.clone()); }
            if let Some(m) = message { a.push("-m".to_string()); a.push(m.clone()); }
            a
        }
        GitAction::CreateBranch { name, from } => {
            let mut a = vec!["checkout".to_string(), "-b".to_string(), name.clone()];
            if let Some(f) = from { a.push(f.clone()); }
            a
        }
        GitAction::Checkout { name } => vec!["checkout".to_string(), name.clone()],
        GitAction::Rebase { upstream, interactive } => {
            let mut a = vec!["rebase".to_string()];
            if interactive.unwrap_or(false) { a.push("-i".to_string()); }
            a.push(upstream.clone());
            a
        }
        GitAction::Merge { branch, no_ff } => {
            let mut a = vec!["merge".to_string()];
            if no_ff.unwrap_or(false) { a.push("--no-ff".to_string()); }
            a.push(branch.clone());
            a
        }
        GitAction::RemoteAdd { name, url } => {
            vec!["remote".to_string(), "add".to_string(), name.clone(), url.clone()]
        }
        GitAction::Tag { name, sha, message } => {
            let mut a = vec!["tag".to_string()];
            if let Some(m) = message { a.push("-m".to_string()); a.push(m.clone()); }
            a.push(name.clone());
            if let Some(s) = sha { a.push(s.clone()); }
            a
        }
    };
...
mod tests {
    ...
    #[test]
    fn build_stash_argv() {
        let action = GitAction::Stash { sub: Some("push".into()), message: Some("WIP".into()) };
