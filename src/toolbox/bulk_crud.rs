...
fn run_parallel(base: &Path, ops: &[Op], max_bytes: usize) -> Vec<serde_json::Value> {
    ...
    let handles: Vec<_> = owned.into_iter().map(|op| {
        let tx = tx.clone();
        let base = base.clone();
    ...
    match op {
        Op::Read { path } => {
            ...
            match std::fs::read_to_string(&full) {
                ...
                Err(e) => serde_json::json!({"op":"read","path":path,"ok":false,"error":e.to_string()}),
            }
        }
        Op::Write { path, content } => {
            ...
            match std::fs::write(&full, content) {
                Ok(()) => serde_json::json!({"op":"write","path":path,"ok":true,"bytes":content.len()}),
                Err(e) => serde_json::json!({"op":"write","path":path,"ok":false,"error":e.to_string()}),
            }
        }
        Op::Delete { path } => {
            let full = base.join(path);
            match std::fs::remove_file(&full) {
                ...
                Err(e) => serde_json::json!({"op":"delete","path":path,"ok":false,"error":e.to_string()}),
            }
        }
        Op::Move { from, to } => {
            ...
            match std::fs::rename(&f, &t) {
                ...
                Err(e) => serde_json::json!({"op":"move","from":from,"to":to,"ok":false,"error":e.to_string()}),
            }
        }
        Op::Copy { from, to } => {
            let f = base.join(from);
            ...
            match std::fs::copy(&f, &t) {
                Ok(bytes) => serde_json::json!({"op":"copy","from":from,"to":to,"ok":true,"bytes":bytes}),
                Err(e) => serde_json::json!({"op":"copy","from":from,"to":to,"ok":false,"error":e.to_string()}),
            }
        }
    }
}
...
mod tests {
    ...
    fn bulk_write_read() {
        ...
        let args = Args {
            operations: vec![
                Op::Write { path: "a.txt".into(), content: "alpha".into() },
                Op::Write { path: "b.txt".into(), content: "beta".into() },
                Op::Read { path: "a.txt".into() },
                Op::Read { path: "b.txt".into() },
            ],
            parallel: false,
            stop_on_error: true,
            max_bytes: Some(1000),
