...
pub fn build_lines(todos: &[Todo], focused: bool, width: u16) -> Vec<Line<'static>> {
    ...
    if todos.is_empty() {
        out.push(Line::styled(
            ...
            Style::default().fg(t.text_muted).bg(t.bg_secondary).add_modifier(Modifier::ITALIC),
        ));
        out.push(Line::default());
        out.push(Line::from(vec![
            Span::styled("  /todo add ", Style::default().fg(t.accent_primary).bg(t.bg_secondary).add_modifier(Modifier::BOLD)),
            Span::styled("create", Style::default().fg(t.text_muted).bg(t.bg_secondary)),
        ]));
        return out;
    }
        ...
        for (j, line) in wrapped.into_iter().enumerate() {
            if j == 0 {
                let text_style = if is_done {
                    Style::default().fg(t.text_muted).bg(t.bg_secondary).add_modifier(Modifier::CROSSED_OUT)
                } else {
                    Style::default().fg(if focused { t.text_primary } else { t.text_secondary }).bg(t.bg_secondary)
                };
                out.push(Line::from(vec![
                    Span::styled(format!(" {num} "), Style::default().fg(t.text_muted).bg(t.bg_secondary)),
    ...
    // footer hints
    out.push(Line::from(vec![
        Span::styled("  /todo ", Style::default().fg(t.accent_primary).bg(t.bg_secondary).add_modifier(Modifier::BOLD)),
        Span::styled("add/done/rm/clear", Style::default().fg(t.text_muted).bg(t.bg_secondary)),
    ]));
    out.push(Line::from(vec![
        Span::styled("  ? ", Style::default().fg(t.accent_secondary).bg(t.bg_secondary).add_modifier(Modifier::BOLD)),
        Span::styled("shortcuts", Style::default().fg(t.text_muted).bg(t.bg_secondary)),
    ]));
...
mod tests {
    ...
    fn empty_shows_hint() {
        ...
        let flat: String = lines.iter().map(|l| l.spans.iter().map(|s| s.content.clone()).collect::<String>()).collect::<Vec<_>>().join("\n");
        assert!(flat.contains("No tasks"));
    }

    #[test]
    fn renders_mixed() {
        let todos = vec![Todo { text: "a".into(), done: false }, Todo { text: "b".into(), done: true }];
